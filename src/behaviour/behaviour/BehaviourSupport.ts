...
export class BehaviourSupport {
  ...
  setTimeWhenDark() {
    this.rule.data.time = { type: "RANGE", from: {type:"SUNSET", offsetMinutes:0}, to: {type:"SUNRISE", offsetMinutes:0} };
    return this;
  }
  setTimeWhenSunUp() {
    this.rule.data.time = { type: "RANGE", from: {type:"SUNRISE", offsetMinutes:0}, to: {type:"SUNSET", offsetMinutes:0} };
    return this;
  }
  setTimeFromSunrise(offsetMinutes : number = 0) {
    ...
    if (this.rule.data.time.type === "ALL_DAY") { this.setTimeWhenSunUp(); }
  ...
  setTimeFromSunset(offsetMinutes : number = 0) {
    // if the time was ALL_DAY, set it to an acceptable range, given the name of this method.
    if (this.rule.data.time.type  === "ALL_DAY") { this.setTimeWhenDark(); }
  ...
  setPresenceSomebodyInLocations(locationIds: number[]) : BehaviourSupport {
    this.rule.data.presence = { type:"SOMEBODY", data: {type:"LOCATION", locationIds: locationIds}, delay: SPHERE_DELAY};
    return this;
  }
  setPresenceNobodyInLocations(locationIds: number[]) : BehaviourSupport {
    this.rule.data.presence = { type:"NOBODY", data: {type:"LOCATION", locationIds: locationIds}, delay: SPHERE_DELAY};
    return this;
  }
  ...
  setEndConditionWhilePeopleInSphere() : BehaviourSupport {
    this.rule.data.endCondition = {type:"PRESENCE_AFTER", presence: {type: "SOMEBODY", data: { type: "SPHERE"}, delay: SPHERE_DELAY}};
    return this;
  }
  setEndConditionWhilePeopleInLocation(locationId: number) : BehaviourSupport {
    this.rule.data.endCondition = {type:"PRESENCE_AFTER", presence: {type: "SOMEBODY", data: { type: "LOCATION", locationIds:[locationId]}, delay: SPHERE_DELAY}};
    return this;
  }
  ...
  hasNoEndCondition(): boolean {
    return !this.rule.data.endCondition;
  }
